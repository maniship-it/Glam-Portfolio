...
Photography in `client/public/images` is served through responsive derivatives
generated by a script, not by shipping the full-size originals to phones:

```bash
npm run optimize:images          # add new photos, then run this
npm run optimize:images -- --force   # re-encode everything
```

It writes `name-400.webp` / `-800.webp` / `-1200.webp` next to each original and
records what exists in `client/src/lib/image-manifest.json`, which drives the
`srcset` helpers in `client/src/lib/images.ts` and the hero preload tag. The
originals are untouched and are still what the gallery lightbox opens.

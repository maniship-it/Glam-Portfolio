    ...
    <>
      ...
      >
        <div className="container mx-auto px-4 md:px-8 flex items-center justify-between">
          <a href="#" onClick={closeMobileMenu} className="flex items-center gap-2 group z-50">
          ...
          <nav className="hidden md:flex items-center gap-8 text-sm font-medium">
            <a 
              href="#portfolio" 
              className={`transition-all duration-300 ${activeSection === 'portfolio' ? 'text-primary drop-shadow-[0_0_10px_rgba(216,195,165,0.8)] scale-105' : 'text-gray-300 hover:text-primary'}`}
            >
              Portfolio
            </a>
            <a 
              href="#services" 
              className={`transition-all duration-300 ${activeSection === 'services' ? 'text-primary drop-shadow-[0_0_10px_rgba(216,195,165,0.8)] scale-105' : 'text-gray-300 hover:text-primary'}`}
            >
              Services
            </a>
            <a 
              href="#testimonials" 
              className={`transition-all duration-300 ${activeSection === 'testimonials' ? 'text-primary drop-shadow-[0_0_10px_rgba(216,195,165,0.8)] scale-105' : 'text-gray-300 hover:text-primary'}`}
            >
              Testimonials
            </a>
          </nav>
        ...
        <div className={`fixed inset-0 bg-background/95 backdrop-blur-xl z-40 transition-transform duration-300 ease-in-out flex flex-col justify-center items-center md:hidden ${mobileMenuOpen ? 'translate-x-0' : 'translate-x-full'}`}>
          <nav className="flex flex-col items-center gap-8 text-xl font-serif">
            <a href="#portfolio" onClick={closeMobileMenu} className="text-gray-300 hover:text-primary transition-colors">Portfolio</a>
